...
DOCKERFILE = Path(__file__).resolve().parents[1] / "Dockerfile.facade-ml"


class FacadeMlDownloadsTest(unittest.TestCase):
    def test_bootstraps_pip_before_using_resumable_downloads(self):
        ...
        self.assertIn(
            "pip install --resume-retries 20 --timeout 120 torch==2.5.1",
            dockerfile,
        )
        self.assertIn(
