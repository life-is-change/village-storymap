(function () {
  ...
  async function hasAny3DBuildingRowsForSpace(spaceId) {
    ...
    if (error) {
      console.warn("检查 3D 空间 building 是否已初始化失败：", error);
      return false;
    }
  ...
  async function showEntityInfo(entity) {
    ...
    infoPanel.innerHTML = `
      ...
      <div class="info-card">
        ...
        <div class="house-row">当前空间：${escapeHtml(document.getElementById('statusBadge')?.textContent?.split('空间：')[1]?.split('｜')[0] || '村庄现状')}</div>
  ...
  async function initViewer() {
    ...
    const localHighResLayer = viewer.imageryLayers.addImageryProvider(
      new Cesium.SingleTileImageryProvider({
        url: "assets/orthophoto_georef.png",
        rectangle: localRect
      })
    );
