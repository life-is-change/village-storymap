(function () {
  ...
  const ONLINE_RESOURCE_TIMEOUT_MS = 20000;
  ...
  function resolveFirstPersonSpawnCartographic() {
    ...
    const centerLon = (georef.minX + georef.maxX) / 2;
    const centerLat = (georef.minY + georef.maxY) / 2;
  ...
  async function addViewerImageryLayers(canUseIonServices) {
    ...
    if (!hasGlobalOnlineImagery) {
      // ArcGIS 不可达时，再尝试 OSM 公共瓦片，避免直接落到绿色底图
      try {
        const osmProvider = await withTimeout(
        ...
        hasImagery = true;
        hasGlobalOnlineImagery = true;
        console.log("OSM 在线底图加载成功");
      } catch (osmError) {
        console.warn("OSM 在线底图加载失败：", osmError?.message || osmError);
        // 所有在线底图都失败时，确保 NaturalEarthII 仍然可见作为最终回退
        if (naturalEarthLayer) {
  ...
  async function initViewer() {
    ...
    // 尝试加载地形：先用用户 token，再用 Cesium 默认 token，然后尝试 ArcGIS，最后回退到椭球体
    ...
    if (canUseIonServices) {
      try {
        ...
        console.warn("使用自定义 token 加载在线地形失败：", error?.message || error);
        // 尝试用 Cesium 内置默认 token 重试一次
        try {
          ...
          console.log("使用 Cesium 默认 token 在线地形加载成功");
        } catch (error2) {
          console.warn("使用默认 token 加载在线地形也失败：", error2?.message || error2);
          console.warn("提示：Cesium Ion 服务访问受限或 token 已失效，建议前往 https://ion.cesium.com/ 生成新 Access Token 并替换 index.html 中的 window.CESIUM_ION_TOKEN。");
          ionServicesLikelyBlocked = true;
          canUseIonServices = false;
        }
      }
    }
    ...
    if (!terrainProvider) {
      try {
        ...
        console.log("ArcGIS 在线地形加载成功");
      } catch (arcGisTerrainError) {
        console.warn("ArcGIS 在线地形加载失败：", arcGisTerrainError?.message || arcGisTerrainError);
      }
    }

    // 即使在线地形失败，也显式使用 EllipsoidTerrainProvider 保证 globe 正常渲染
    if (!terrainProvider) {
