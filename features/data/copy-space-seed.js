(function () {
  ...
  async function upsertInChunks(supabaseClient, tableName, payload, chunkSize = 200) {
    ...
    // Live Server/本地教学模式下不依赖 Supabase，直接写入 localStorage。
    if (!supabaseClient) {
      if (window.FeatureDbModule && typeof window.FeatureDbModule.__upsertLocalRows === "function") {
        window.FeatureDbModule.__upsertLocalRows(null, rows);
      }
      return;
    }
  ...
  async function fetchBaseSpaceDbRows(deps, spaceId, layerKey) {
    ...
    if (!supabaseClient) {
      if (window.FeatureDbModule && typeof window.FeatureDbModule.__listLocalRows === "function") {
        return window.FeatureDbModule.__listLocalRows(spaceId, layerKey) || [];
      }
      return [];
    }
    ...
    if (error) {
      console.warn(`读取现状空间 ${layerKey} 数据库数据失败，已尝试回退本地缓存：`, error);
      if (window.FeatureDbModule && typeof window.FeatureDbModule.__listLocalRows === "function") {
        return window.FeatureDbModule.__listLocalRows(spaceId, layerKey) || [];
      }
      return [];
