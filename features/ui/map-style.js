  ...
  const api = {
    getOlFeatureStyle(deps, feature, resolution) {
      ...
      if (layerKey === "communityTask") {
        ...
        const category = taskRow.category || "garbage";
        let color;
        if (status === "pending") {
        ...
        if (typeof CircleStyle !== "function") {
          return new Style({
            ...
            stroke: new Stroke({ color, width: isActive ? 3 : 2 }),
        ...
        return new Style({
          ...
          image: new CircleStyle({
            radius: isActive ? 12 : (isHovered ? 8 : 7),
            fill: new Fill({ color: `${color}cc` }),
            stroke: new Stroke({ color: isActive ? "#ffffff" : "#ffffff", width: isActive ? 3 : 2 })
          })
        });
      }
