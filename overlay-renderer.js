  ...
  const api = {
    async refresh2DOverlay(deps) {
      ...
      for (const layerKey of effectiveLayerKeys) {
        try {
          if (layerKey === "building" && currentSpaceId !== deps.BASE_SPACE_ID) {
          ...
          if (layerKey === "road" && currentSpaceId !== deps.BASE_SPACE_ID) {
          ...
          if ((layerKey === "cropland" || layerKey === "openSpace") && currentSpaceId !== deps.BASE_SPACE_ID) {
      ...
      await deps.refreshCommentHighlightsForCurrentSpace(effectiveLayerKeys);
